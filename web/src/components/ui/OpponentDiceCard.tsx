...
export function OpponentDiceCard({ name, lives, diceRemaining, isEliminated = false }: OpponentDiceCardProps) {
  return (
    <motion.div
    ...
    >
      ...
      <div className={`flex flex-col items-center gap-2 ${isEliminated ? 'opacity-30' : ''}`}>
        ...
        {/* Dice Remaining */}
        {!isEliminated && (
          <div className="flex items-center gap-1 mt-1 text-xs font-bold font-mono text-cyan-400">
            <Dices className="w-4 h-4" />
            <span>{diceRemaining} DADOS</span>
          </div>
        )}
      </div>
    </motion.div>
